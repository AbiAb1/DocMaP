...
$sql = "
    SELECT 
        ...
        ts.taskContent, 
        ts.TimeStamp, 
        tu.Status AS TaskUserStatus, 
        uc.Status AS UserContentStatus, 
    ...
    WHERE 
        tu.UserID = $user_id 
        AND tu.Status IN ('Assigned', 'Missing') 
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        // Assuming DueDate is in 'Y-m-d' format and DueTime is in 'H:i:s' format
...
if ($row['TaskUserStatus'] === 'Assigned') {
...
} elseif ($row['TaskUserStatus'] === 'Missing') {
